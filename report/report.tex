...
With $q = 50$, the generated images are blurry and lack structure, indicating insufficient capacity to capture the data distribution. At $q = 100$, recognizable digit shapes begin to emerge. With $q = 200$ and $q = 300$, the generated images are sharp and closely resemble the training data. Beyond $q = 200$, there is no significant visual improvement, suggesting that 200 hidden units provide sufficient representational capacity for this dataset.
...
With 3 characters, the RBM produces clean, recognizable digits. As we increase to 10 and 36 characters, the generated images become progressively noisier and less distinct. This is expected: with only 39 samples per character and a fixed hidden layer size, the model must represent a much more diverse distribution. The limited data per class combined with the increased variety leads to a less sharp generative model. This highlights a fundamental limitation of RBMs with fixed capacity when the complexity of the target distribution increases.
...
The single-layer DBN (equivalent to an RBM) produces sharp images. The 2-layer DBN generates images of comparable quality, showing that the additional layer of abstraction does not degrade performance. However, the 3-layer DBN produces slightly less crisp images. This suggests that for this small dataset, deeper architectures do not bring additional benefit and may even slightly hurt generation quality, likely because the top RBM operates on a very low-dimensional space ($50$ units) with limited expressiveness for Gibbs sampling.
...
The trend is similar to the RBM experiments: image quality degrades with more character classes. The DBN shows a modest improvement over the single-layer RBM when handling more diverse data, thanks to the hierarchical feature representation. Nevertheless, the overall limitation remains: with 39 samples per class and increasing diversity, the model cannot fully capture the distribution.
...
\begin{figure}[H]
    ...
    \caption{Classification error rate on the test set as a function of the number of hidden layers (200 neurons each). Blue: pre-trained + backprop. Red: random init + backprop.}
    \label{fig:fig1}
\end{figure}
...
\textbf{Analysis.} The pre-trained network improves consistently with more neurons, reaching its best performance ($\approx 2.6\%$) at 700 neurons per layer. The randomly initialized network shows diminishing returns and plateaus around $\approx 2.8\%$ regardless of width.

Pre-training benefits wider networks because the unsupervised phase learns increasingly rich feature representations with more hidden units. The randomly initialized network, lacking this structured initialization, cannot fully exploit the additional capacity. For our best model, \textbf{700 neurons per layer} is the best choice.
...
\textbf{Analysis.} Pre-training provides its greatest advantage in the \textbf{low-data regime}. With only 1000 training samples, the gap between pre-trained and random is substantial. As the dataset grows to 60,000 samples, both networks converge to similar performance.

This is consistent with theory: unsupervised pre-training acts as a regularizer by learning the structure of the input distribution. When labeled data is scarce, this prior knowledge significantly helps generalization. With abundant labeled data, supervised training alone can learn comparable features. For our best model, we use \textbf{all 60,000 training samples}.
...
The best model achieves a test error rate of approximately \textbf{2.5--3\%}. The softmax output probabilities (Figure~\ref{fig:softmax}) show confident and well-calibrated predictions, with the probability mass concentrated on the correct class for most samples.
...
\begin{figure}[H]
    ...
    \caption{Generated MNIST samples from five generative models with approximately the same parameter budget.}
    \label{fig:bonus}
\end{figure}
...
\begin{enumerate}
    ...
    \item The \textbf{optimal configuration} for MNIST classification is $[784, 700, 700, 10]$ with pre-training, achieving $\approx 2.5\%$ test error.
